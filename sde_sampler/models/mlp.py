...
class FourierMLP(Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.step_conditioned = step_conditioned
    ...
    def forward(
        ...
        if self.step_conditioned:
            d = d.view(-1, 1).expand(x.shape[0], 1).float()
            embed_d = self.step_embed(d)
            assert embed_d.shape == embed_x.shape
            embed += embed_d
