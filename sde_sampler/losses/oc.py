...
class BaseOCLoss:
    ...
    def generative_and_sde_ctrl(
        self, t: torch.Tensor, x: torch.Tensor, d: torch.Tensor | None = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
        generative_ctrl = self.generative_ctrl(t, x, d)
...
class TimeReversalLoss(BaseOCLoss):
    def __init__(
        ...
        *args,
        ...
        is_step_conditioned: bool = False,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.inference_ctrl = inference_ctrl
        self.div_estimator = div_estimator
        self.is_step_conditioned = is_step_conditioned
    ...
    def simulate(
        ...
        for s, t in zip(ts[:-1], ts[1:]):
            # Evaluate
            dt = t - s
            if self.is_step_conditioned: 
                d = dt 
            else: 
                d = None
            if change_sde_ctrl:
                generative_ctrl, sde_ctrl = self.generative_and_sde_ctrl(s, x, d)
            else:
                sde_ctrl = generative_ctrl = self.generative_ctrl(s, x, d)
            ...
            if ode:
                x = x + (self.sde.drift(s, x) + 0.5 * sde_diff * sde_ctrl) * dt
            else:
    ...
    def __call__(
        ...
        if return_traj:
            return self.compute_loss(rnd, samples=samples), xs
        else:
...
class ForwardBackwardRND(BaseOCLoss):
    ...
    def simulate(
        ...
        for s, t in zip(ts[:-1], ts[1:]):
            ...
            # Evaluate forward control.
            forward_ctrl = self.generative_ctrl(s, x)  # shape (batch_size, d)
            if change_sde_ctrl:
                sde_ctrl = forward_ctrl.detach()
            else:
                sde_ctrl = forward_ctrl

            # Forward step (Euler–Maruyama).
            drift = self.sde.drift(s, x)
            diff = self.sde.diff(s, x)  # shape (batch_size, d)
            forward_mean = (drift + diff * forward_ctrl) * dt
            forward_scale = diff * dt.sqrt()
            x_next = sample_kernel(
                x + (drift + diff * sde_ctrl) * dt, forward_scale
            )
            
            # Compute log p_{n|n+1}( x | x_next ).
            drift_next = self.sde.drift(t, x_next)
            diff_next = self.sde.diff(t, x_next)
            backward_mean = drift_next * dt  # inference_ctrl v = 0 
            backward_scale = diff_next * dt.sqrt()
            
            delta_x = (x_next - x)
            log_p_forward = gaussian_logpdf(
                delta_x, forward_mean, forward_scale
            )
            log_p_backward = gaussian_logpdf(
                -delta_x, backward_mean, backward_scale
            )

            rnd += log_p_forward - log_p_backward
        ...
        rnd -= terminal_unnorm_log_prob(x)  # shape (batch_size, 1)

        if return_traj:
            xs = torch.stack(xs, dim=0)  # shape (N+1, batch_size, d)
        return x, rnd, xs
...
def gaussian_logpdf(
    x: torch.Tensor, mean: torch.Tensor, scale: torch.Tensor
) -> torch.Tensor:
    dist = Independent(Normal(mean, scale), 1)  # Independent across last dim
    return dist.log_prob(x).unsqueeze(-1)  # shape: (batch_size, 1)

def sample_kernel(mean, scale):
